...
def test_client_from_spec(fixture_dir):
  ...
  assert bravado.client.CallableOperation is type(c.api.v1.whoami)
...
def test_client_from_spec_error(fixture_dir):
  ...
  assert '<ClientError code=[71] message=schema validation failed data=None>' == repr(err)
...
def test_client_from_url_parse_error(fixture_dir):
  with open(f'{fixture_dir}/spec.json') as f, requests_mock.mock() as m, pytest.raises(errors.ClientError) as e:
    m.get('http://myspec.com', text='{somethin: {wrong:')
    c = Client.from_url('http://myspec.com')
  err = e.value

  assert 'remote data validation failed' == str(err)
  assert '<ClientError code=[71] message=remote data validation failed data=None>' == repr(err)
...
def test_client_from_url_response_error(fixture_dir):
  ...
  assert 'error loading remote spec data' == str(err)
  assert {'url': 'http://myspec.com/', 'method': 'GET', 'status_code': 403} == err.data