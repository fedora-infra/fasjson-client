...
class Client(object):
  ...
  def __init__(self, spec, principal):
    ...
    try:
      self.api = SwaggerClient.from_spec(spec, http_client=self.http_client, config={'disable_fallback_results': True})
  ...
  def from_spec(cls, spec_data, base_url=None, principal=None):
    ...
    if base_url:
      ...
      if not parsed.netloc or not parsed.scheme:
        ...
        raise errors.ClientError(f'unable to parse base_url: {base_url}', errno.EPROTO, data=data)
